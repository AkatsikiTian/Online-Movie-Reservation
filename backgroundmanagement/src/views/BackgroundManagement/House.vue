<template>
  <div v-title data-title="晓天电影订票后台管理系统-影厅管理">
    ...
    <el-table :data="tableData" border stripe :header-cell-style="{'text-align':'center'}"
      ...
      <el-table-column prop="id" label="影厅编号" width="140">
      </el-table-column>
      <el-table-column label="影厅名称" prop="name">
      </el-table-column>
      <el-table-column prop="size" label="影厅容量">
      </el-table-column>
    ...
    <el-dialog title="新增或修改影厅(请单击确定保存)" :visible.sync="dialogvisible" width="30%">
      <el-form :model="form" status-icon  ref="form" :rules="rules">
        <el-form-item label="影厅名称" :label-width="formLabelWidth" prop="name">
          <el-input v-model="form.name" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="影厅大小" :label-width="formLabelWidth" prop="size">
          <el-radio-group v-model="form.size">
            <el-radio :label="40">40个座位</el-radio>
            <el-radio :label="60">60个座位</el-radio>
          </el-radio-group>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    save(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          request.post("/house", this.form).then(
              res => {
